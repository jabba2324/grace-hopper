...
async function gqlProjects(owner: string): Promise<Array<{ number: number; title: string }>> {
    ...
    try {
        ...
        const nodes = JSON.parse(raw)?.data?.user?.projectsV2?.nodes as Array<{ number: number; title: string }> ?? [];
